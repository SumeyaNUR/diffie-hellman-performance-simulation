...
def scale_series_to_target_variance(x: np.ndarray, target_var: float) -> np.ndarray:
    ...
    """
    if x.size < 2:
        raise ValueError("Need at least 2 samples to define sample variance")

    x0 = x - x.mean()

    # Current sample variance with ddof=1
    current_var = x0.var(ddof=1)
    if current_var == 0:
        raise ValueError("Current variance is zero; cannot scale.")

    # Scale factor so new variance matches target
...
def generate_dataset(cfg: CSIConfig) -> pd.DataFrame:
    ...
    amp_normal = scale_series_to_target_variance(amp_normal_raw, cfg.target_normal_variance)
    amp_attack = scale_series_to_target_variance(amp_attack_raw, cfg.target_attack_variance)
    ...
    phase = (phase + np.pi) % (2 * np.pi) - np.pi  # wrap to [-pi, pi]
